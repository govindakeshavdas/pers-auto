...
# context at all. The single-call version this replaced ran 30 searches in one
# context and carried the whole ledger in its prompt.
#
...
anthropic_call "$discover_prompt" "$DISCOVER_MAX_SEARCHES" low 8000 /tmp/funding_discover_response.json
...
candidates=$(response_text /tmp/funding_discover_response.json | python3 -c '
import re, sys

ledger = {line.strip().lower() for line in open(sys.argv[1]) if line.strip()}

# An anonymous company is not a candidate. Its "name" is invented, so it never
# matches the ledger and the same round resurfaces every run — and evaluate
...
if [ "$n_candidates" -eq 0 ]; then
  ...
  response_text /tmp/funding_discover_response.json | head -c 2000 >&2
  exit 1
fi
if [ "$n_candidates" -lt 4 ]; then
  echo "--- WARNING: only $n_candidates candidates; expect a thin run ---" >&2
fi
...
printf '%s\n' "$candidates" > /tmp/funding_candidates.txt
...
anthropic_call "$triage_prompt" 0 medium 4000 /tmp/funding_triage_response.json

shortlist=$(response_text /tmp/funding_triage_response.json | python3 -c '
import re, sys
...
anthropic_call "$eval_prompt" "$EVALUATE_MAX_SEARCHES" medium 40000 /tmp/funding_evaluate_response.json
...
raw=$(response_text /tmp/funding_evaluate_response.json)
